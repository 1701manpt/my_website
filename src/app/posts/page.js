...
export default function PostsPage() {
  ...
  return (
    <div className="grid xl:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 grid-flow-row gap-4">
      {dataPosts.map((post) => (
        <div key={post.id} className="bg-white p-4 rounded shadow-lg">
          ...
          <Link href={"/posts/" + post.id} className="font-medium line-clamp-3">
            {post.title}
          </Link>
        </div>
      ))}
